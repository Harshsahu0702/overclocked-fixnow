...
const Landingpage = ({ onLoginClick }) => {
    ...
    useEffect(() => {
        const fetchExpertCount = async () => {
            try {
                const res = await axios.get('http://192.168.121.253:5000/api/partners/available-count');
                if (res.data.success) {
    ...
    const handleCompletePayment = async () => {
        setLoading(true);
        try {
            const res = await axios.patch(`http://192.168.121.253:5000/api/jobs/${activeJob._id}/status`, { status: 'PAID' });
            if (res.data.success) {
                setActiveJob(null);
                setViewState('idle');
                setAiResult(null);
