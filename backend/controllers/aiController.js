...
exports.interpretService = async (req, res) => {
    try {
        ...
        // Filter workers who match the detected service skill
        const matchedWorkers = workers.filter(w => {
            if (!w.skills || w.skills.length === 0) return false;
            const searchTerm = detectedService.toLowerCase();
            return w.skills.some(s => s.toLowerCase().includes(searchTerm) || searchTerm.includes(s.toLowerCase()));
        });
